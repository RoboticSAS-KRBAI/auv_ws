...
class Subscriber():
    def __init__(self):
        ...
        self.pid_pitch = PID(2000, 0, 500) # P 2000 I 0 D 500 jika ingin pakai thrust 9 & 10
        
        self.pid_yaw = PID(6, 0, 0)

        # self.pid_yaw = PID(1000, 0, 200)
        ...
        rospy.Subscriber('constrain_pwm', Float32, self.callback_constrain_pwm)
        # rospy.Subscriber('is_stable', IsStable, self.callback_is_stable)

        self.pub_dive = rospy.Publisher('dive',Bool,queue_size=10)

    def callback_constrain_pwm(self,data:Float32):
        self.pwm_min = data.data
        self.pwm_max = (1500-data.data) + 1500
    ...
    def surge_sway_yaw(self):
        ...
        pwm_thruster_9 = self.constrain(1500 - (self.thrust_surge_sway_yaw[0] * 500 - self.get_offset(self.offset_surge_sway_yaw[0])), self.pwm_min, self.pwm_max)
        #self.pwm_surge
        pwm_thruster_10 = self.constrain(1500 - (self.thrust_surge_sway_yaw[1] * 500 - self.get_offset(self.offset_surge_sway_yaw[1])), self.pwm_min, self.pwm_max)
        #self.pwm_surge
    ...
    def stabilize_surge_sway_yaw(self, error):
        self.t_surge_sway_yaw = np.interp(self.pid_yaw(error), [-500, 500], [-3, 3])
        self.thrust_surge_sway_yaw = self.ssyController.control(2, self.t_surge_sway_yaw, self.t_surge_sway_yaw)
